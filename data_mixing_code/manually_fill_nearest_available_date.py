...
upper_date = str(int(base_year[:4])+1) + '-12-31'
lower_date = str(int(base_year[:4])-1) + '-12-31'
...
for row in missing_rows:
    ...
    if row_upper in combined_data.index:
        ...
        missing_rows_with_nearest_date.append(combined_data.loc[row_upper])
    elif row_lower in combined_data.index:
        #if so, then we can add that row and its vlaues from combined data to the list
        missing_rows_with_nearest_date.append(combined_data.loc[row_lower])
...
combined_data_filled['Source'] = combined_data_filled['Dataset']
#change dataset to 'Nearest available date'
combined_data_filled['Dataset'] = 'Nearest_av_date'
