...
for measure in data_coverage['Measure'].unique():
    for transport_type in data_coverage['Transport Type'].unique():
        ...
        for i,economy in enumerate(sorted(data_coverage['Economy'].unique())):
            ...
            current_economy_measure_data = data_coverage.loc[(data_coverage['Economy'] == economy) & (data_coverage['Measure'] == measure) & (data_coverage['Transport Type'] == transport_type)]
        ...
        number_of_values = len(data_coverage.loc[(data_coverage['Measure'] == measure) & (data_coverage['Transport Type'] == transport_type), 'Value'].dropna())   
        ...
        if number_of_values == 0:
            plt.savefig('plotting_output/plot_data_coverage/all_data/NOVALUES_{}_{}_{}_plot.png'.format(FILE_DATE_ID, measure, transport_type))
        else:
            plt.savefig('plotting_output/plot_data_coverage/all_data/{}_{}_{}_plot.png'.format(FILE_DATE_ID, measure, transport_type))
    # if measure == 'energy_use_tonnes' and transport_type == 'combined':
    #     break
#ABOVE THE ISSUE IS:
