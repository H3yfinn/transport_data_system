...
energy_activity_tall['Scope'] = 'all'
...
#    'passengers_or_tonnes', 'Km', 'Km_per_PJ', 'PJ_per_activity'
...
# %%
energy_activity_for_system = energy_activity_tall[energy_activity_tall['Economy'].notna()]

energy_activity_for_system.to_csv('intermediate_data/estimated/eei.csv', index=False)
...
#and do the magnitude conversions for transport_data_system:
magnitude_conversions_from_unit = {
    'stocks': 1/1e6,
    'km_per_pj': 1/1e9,
    'km_per_stock': 1/1e3,
    'passengers_or_tonnes': 1,  
...
transport_data_system['magnitude'] = transport_data_system['unit'].map(magnitude_conversions_from_unit).fillna(1)
...
for measure_medium in all_data[['measure', 'medium']].drop_duplicates().itertuples():
    ...
    fig = px.scatter(data, x='origin', y='value', color='economy', facet_col='vehicle_type', symbol='drive')#, facet_row='economy')
    #)#, facet_row='economy')
    #make each graph have anm indepencent y axis
...
# #first fitler for teh  same combinations of vehicle type, transport type, medium, drive, measure in energy_activity_for_system
# energy_activity_for_system_filter = energy_activity_for_system[['Vehicle Type', 'Transport Type', 'Medium', 'Drive', 'measure']].drop_duplicates()
# transport_data_system = transport_data_system.merge(energy_activity_for_system_filter, on=['Vehicle Type', 'Transport Type', 'Medium', 'Drive', 'measure'], how='inner')
# transport_model = transport_model.merge(energy_activity_for_system_filter, on=['Vehicle Type', 'Transport Type', 'Medium', 'Drive', 'measure'], how='inner')
