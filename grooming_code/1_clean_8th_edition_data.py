#this file is intended for gathering the most useful data from the 8th edition model and fromatting it. the code is long by nature, but an effort has been made to keep it in parts. so look for stocks, efficiency or wahtever data type you need if you need to make a fix or something. 

#note that this file was originally used in the 9th edition model to prepare the data for that, so in some places the comments or code may refer to the 9th edition model, just ignore it. 
#%%
...
stocks_ref = pd.read_excel('input_data/8th_edition_transport_model/raw_data/TransportStocks.xlsx', sheet_name='stocks_Reference')
stocks_cn = pd.read_excel('input_data/8th_edition_transport_model/raw_data/TransportStocks.xlsx', sheet_name='stocks_Net-zero')
...
#%%
#save
stocks_long.to_csv("intermediate_data/8th_edition_transport_model/road_stocks.csv", index=False)
...
activity_file_name = 'intermediate_data/8th_edition_transport_model/activity_from_{}.csv'.format(spreadsheet_name)
InputActivityRatio_file_name = "intermediate_data/8th_edition_transport_model/inputactivityratio_from_{}.csv".format(spreadsheet_name)
...
#%%
#SAVE
energy_nofuel.to_csv("intermediate_data/8th_edition_transport_model/energy.csv", index=False)
energy.to_csv("intermediate_data/8th_edition_transport_model/energy_with_fuel.csv", index=False)
#%%
##############################################################################################################
