...
ATO_data.rename(columns={'ADB members, plus Iran and Russia':'ATO_economy_name'}, inplace=True)
...
#%%
#CREATE AN ALT_MEASURE FOR EACH SET OF MEASURES THAT HELPS TO CATEGORISE ALL OF THEM INTO SIMILAR GROUPS. WE WILL ALSO CHANGE SOME UNITS WHLE WE ARE AT IT
#note that i think we will re do this later on but for now it will help us to group the data into similar categories
...
#manually specifiy the unit here so that we dont have issues where we might accidentally remove the 'million' from the unit or something
bad_unit_names = ['tonne-km', 'ton-km', 'tonne-kilometer travel', 'million tonne-km']
freight_km.loc[freight_km['unit'].isin(bad_unit_names), 'unit'] = 'freight_tonne_km'
...
#manually specifiy the unit here so that we dont have issues where we might accidentally remove the 'million' from the unit or something
bad_unit_names = ['passenger kilometers million',
       'passenger kilometer travel million']
passenger_km.loc[passenger_km['unit'].isin(bad_unit_names), 'unit'] = 'passenger_km'

stocks = ATO_data[ATO_data['measure'].isin(Stocks_indicators)]
if PRINT_GRAPHS_AND_STATS:
    print(stocks['medium'].unique())
    print(stocks['scope'].unique())
    print(stocks['unit'].unique())
    print(stocks['transport_type'].unique())
#create alt_measure column that is just the name of the dataframe
stocks['alt_measure'] = 'Stocks'
#manually specifiy the unit here so that we dont have issues where we might accidentally remove the 'million' from the unit or something
bad_unit_names = ['number of vehicles on road']
stocks.loc[stocks['unit'].isin(bad_unit_names), 'unit'] = 'stocks'

sales = ATO_data[ATO_data['measure'].isin(Sales_indicators)]
if PRINT_GRAPHS_AND_STATS:
    print(sales['medium'].unique())
    print(sales['scope'].unique())
    print(sales['unit'].unique())
    print(sales['transport_type'].unique())
#create alt_measure column that is just the name of the dataframe
sales['alt_measure'] = 'Sales'

registrations = ATO_data[ATO_data['measure'].isin(Registrations)]
if PRINT_GRAPHS_AND_STATS:
    print(registrations['medium'].unique())
    print(registrations['scope'].unique())
    print(registrations['unit'].unique())
    print(registrations['transport_type'].unique())
#create alt_measure column that is just the name of the dataframe
registrations['alt_measure'] = 'Registrations'

special = ATO_data[ATO_data['measure'].isin(Special)]
if PRINT_GRAPHS_AND_STATS:
    print(special['medium'].unique())
    print(special['scope'].unique())
    print(special['unit'].unique())
    print(special['transport_type'].unique())
#create alt_measure column that is just the name of the dataframe
special['alt_measure'] = 'Special'

international = ATO_data[ATO_data['measure'].isin(International)]
if PRINT_GRAPHS_AND_STATS:
    print(international['medium'].unique())
    print(international['scope'].unique())
    print(international['unit'].unique())
    print(international['transport_type'].unique())
#create alt_measure column that is just the name of the dataframe
international['alt_measure'] = 'International'

weird = ATO_data[ATO_data['measure'].isin(Weird)]
if PRINT_GRAPHS_AND_STATS:
    print(weird['medium'].unique())
    print(weird['scope'].unique())
    print(weird['unit'].unique())
    print(weird['transport_type'].unique())
#create alt_measure column that is just the name of the dataframe
weird['alt_measure'] = 'Weird'

#if there is a measure that is not in any of the above lists, print it out
if PRINT_GRAPHS_AND_STATS:
    print('Theses are the measures that are not in any of the lists above and might need to be conisdered:')
    print(ATO_data[~ATO_data['measure'].isin(Freight_km + Passenger_km + Stocks_indicators + Sales_indicators + Registrations + Special + International + Weird)]['measure'].unique())
...
if interpolate:
    ...
    for measure in ATO_data_transport_interp['measure'].unique():
        ...
        if PRINT_GRAPHS_AND_STATS:
            ...
            fig.show()
    print('Setting main dataframe to interpolated dataframe')
