...
# Sometimes the final Year column can be followed by a column called 'Source', Update source or $YEAR$ Index. These will, for now, not be included in the extracted data as it is easier to not include them.
...
#Code | ADB members, plus Iran and Russia | Year | Value | Description | Scope | Mode | Sector | Unit | Indicator Source Code | Source | Website
...
for sheet_name in sheets:
    ...
    #remove Description and Source cols since they have a lot of text in them and arent veryu sueful for this kind of large scale dataframe
...
for file in os.listdir('output_data/ADB_data/'):
    if 'other_data' in file:
        other_data_agg_old = pd.read_csv('output_data/ADB_data/' + file)
...
if same_other_data == False:
    ...
    other_df_agg.to_csv('output_data/ADB_data/ADB_other_data_'+str(datetime.date.today())+'.csv', index=False)

for file in os.listdir('output_data/ADB_data/'):
    if 'actual_data' in file:
        actual_data_agg_old = pd.read_csv('output_data/ADB_data/' + file)
...
if same_actual_data == False:
    ...
    actual_df_agg.to_csv('output_data/ADB_data/ADB_actual_data_'+str(datetime.date.today())+'.csv', index=False)
