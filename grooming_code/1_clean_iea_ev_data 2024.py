...
evs = pd.read_csv('input_data/IEA/IEA Global EV Data 2024.csv')
#%%
...
evs['drive'] = evs['drive'].replace({'BEV':'bev', 'EV':'bev and phev', 'PHEV':'phev', 'FCEV': 'fcev'})
#%%
evs['unit'].unique()#array(['Vehicles', 'percent', 'charging points', 'PJ',
    #    'Milion barrels per day', 'Oil displacement, million lge'],
    #   dtype=object)
    
# convert the gwh values to PJ
evs.loc[evs['unit']=='GWh', 'value'] = evs.loc[evs['unit']=='GWh', 'value'] * 0.0036
#keep all the same but we are going to change gwh to pj
evs['unit'] = evs['unit'].replace({'GWh':'PJ', 'Vehicles':'Stocks'})

#convert stock share and sales share to a proportion by dividing by 100
evs.loc[evs['measure'].isin(['Sales share', 'Stock share']), 'value'] = evs.loc[evs['measure'].isin(['Sales share', 'Stock share']), 'value']/100
#%%
evs['Medium'] = 'road' 
...
#saeve the data
def split_PHEV_and_BEV(STOCKS=True):
    ...
    #the same goes for Sales shares and Sales. We will split them into bev and phev
    """
    if STOCKS:
        #grab stock share data
        evs_stock_share = evs[(evs['measure'] == 'Stock share') & (evs['drive'] == 'bev and phev')]
        #grab stock data
        evs_stock = evs[(evs['measure'] == 'Stocks') & (evs['drive'].isin(['bev', 'phev']))]
    else:
        #grab sales share data
        evs_stock_share = evs[(evs['measure'] == 'Sales share') & (evs['drive'] == 'bev and phev')]
        #grab sales data
        evs_stock = evs[(evs['measure'] == 'Sales') & (evs['drive'].isin(['bev', 'phev']))]
...
evs_sales_share_new.columns = evs_sales_share_new.columns.str.lower()
...
PLOTTING = True
if PLOTTING:
    # Plot sales share in all countries using plotly
    evs_new_sales_share = evs_new[evs_new['measure'] == 'Sales share']
    evs_new_sales_share = evs_new_sales_share[evs_new_sales_share['transport type'] == 'passenger']
    fig = px.line(evs_new_sales_share, x='date', y='value', color='economy', facet_col='vehicle type', line_dash='source', facet_col_wrap=3, facet_row='drive', facet_row_spacing=0.05, facet_col_spacing=0.05, labels={'value':'Sales share of BEV and PHEV'}, title='Sales share of BEV, FCEV and PHEV in all countries')
    fig.update_traces(mode='lines+markers', line=dict(width=3), marker=dict(size=4))
    fig.write_html('plotting_output/analysis/IEA_{}_sales_share.html'.format(FILE_DATE_ID))
    
    # Plot stock share in all countries using plotly (passenger)
    evs_new_stock_share = evs_new[evs_new['measure'] == 'Stock share']
    evs_new_stock_share = evs_new_stock_share[evs_new_stock_share['transport type'] == 'passenger']
    fig = px.line(evs_new_stock_share, x='date', y='value', color='economy', facet_col='vehicle type', line_dash='source', facet_col_wrap=3, facet_row='drive', facet_row_spacing=0.05, facet_col_spacing=0.05, labels={'value':'Stock share of BEV and PHEV'}, title='Stock share of BEV, FCEV and PHEV in all countries')
    fig.update_traces(mode='lines+markers', line=dict(width=3), marker=dict(size=4))
    fig.write_html('plotting_output/analysis/IEA_{}_stock_share_passenger.html'.format(FILE_DATE_ID))
    
    # Plot stock share in all countries using plotly (freight)
    evs_new_stock_share = evs_new[evs_new['measure'] == 'Stock share']
    evs_new_stock_share = evs_new_stock_share[evs_new_stock_share['transport type'] == 'freight']
    fig = px.line(evs_new_stock_share, x='date', y='value', color='economy', facet_col='vehicle type', line_dash='source', facet_col_wrap=3, facet_row='drive', facet_row_spacing=0.05, facet_col_spacing=0.05, labels={'value':'Stock share of BEV and PHEV'}, title='Stock share of BEV, FCEV and PHEV in all countries')
    fig.update_traces(mode='lines+markers', line=dict(width=3), marker=dict(size=4))
    fig.write_html('plotting_output/analysis/IEA_{}_stock_share_freight.html'.format(FILE_DATE_ID))
    
    # Plot oil displacement in all countries using plotly
    evs_new_oil_displacement = evs_new[evs_new['measure'] == 'Oil displacement Mbd']
    fig = px.line(evs_new_oil_displacement, x='date', y='value', color='economy', facet_col='vehicle type', line_dash='source', facet_col_wrap=3, facet_row='drive', facet_row_spacing=0.05, facet_col_spacing=0.05, labels={'value':'Oil displacement Mbd'}, title='Oil displacement Mbd in all countries')
    fig.update_traces(mode='lines+markers', line=dict(width=3), marker=dict(size=4))
    fig.write_html('plotting_output/analysis/IEA_{}_oil_displacement.html'.format(FILE_DATE_ID))
    
    # Plot electricity demand in all countries using plotly
    evs_new_electricity_demand = evs_new[evs_new['measure'] == 'Energy']
    fig = px.line(evs_new_electricity_demand, x='date', y='value', color='economy', facet_col='vehicle type', line_dash='source', facet_col_wrap=3, facet_row='drive', facet_row_spacing=0.05, facet_col_spacing=0.05, labels={'value':'Energy (PJ)'}, title='Energy demand in all countries')
    fig.update_traces(mode='lines+markers', line=dict(width=3), marker=dict(size=4))
    fig.write_html('plotting_output/analysis/IEA_{}_energy.html'.format(FILE_DATE_ID))
    
    # Plot sales in all countries using plotly
    evs_new_sales = evs_new[evs_new['measure'] == 'Sales']
    fig = px.line(evs_new_sales, x='date', y='value', color='economy', facet_col='vehicle type', line_dash='source', facet_col_wrap=3, facet_row='drive', facet_row_spacing=0.05, facet_col_spacing=0.05, labels={'value':'Sales'}, title='Sales in all countries')
    fig.update_traces(mode='lines+markers', line=dict(width=3), marker=dict(size=4))
    fig.write_html('plotting_output/analysis/IEA_{}_sales.html'.format(FILE_DATE_ID))
    
    # Plot stocks in all countries using plotly
    evs_new_stocks = evs_new[evs_new['measure'] == 'Stocks']
    fig = px.line(evs_new_stocks, x='date', y='value', color='economy', facet_col='vehicle type', line_dash='source', facet_col_wrap=3, facet_row='drive', facet_row_spacing=0.05, facet_col_spacing=0.05, labels={'value':'Stocks'}, title='Stocks in all countries')
    fig.update_traces(mode='lines+markers', line=dict(width=3), marker=dict(size=4))
    fig.write_html('plotting_output/analysis/IEA_{}_stocks.html'.format(FILE_DATE_ID))

    # Plot EV Charging points in all countries using plotly
    evs_new_ev_charging_points = evs_new[evs_new['measure'] == 'EV Charging points']
    fig = px.line(evs_new_ev_charging_points, x='date', y='value', color='economy', facet_col='vehicle type',line_dash='source',  facet_col_wrap=3, facet_row='drive', facet_row_spacing=0.05, facet_col_spacing=0.05, labels={'value':'EV Charging points'}, title='EV Charging points in all countries')
    fig.update_traces(mode='lines+markers', line=dict(width=3), marker=dict(size=4))
    fig.write_html('plotting_output/analysis/IEA_{}_ev_charging_points.html'.format(FILE_DATE_ID))
