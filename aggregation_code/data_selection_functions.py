        ...
        for year in data_for_plotting.Date.unique():
                ...
                if user_input == 'quit':
                        ...
                        if option == options[0]:
                                ...
                                data_to_change = data_to_change[data_to_change.Dataset == dataset]
                                #filter for years after the current year
                                data_to_change = data_to_change[data_to_change.Date >= year]
                                ...
                                if test_x:
                                        ...
                                        data_to_change = data_to_change[~((data_to_change.Date == 2020) & (data_to_change.Dataset == '8th edition transport model (forecasted reference scenario)'))]
...
test =False
if test:
        #PLACE THIS ON ROW 129 (or just type into a cell) of 3_select_data.py
        #save input to function to pickle file
        # import pickle
        # with open('intermediate_data/data_selection/automatic_method_input.pkl', 'wb') as f:
        #        pickle.dump([combined_data_automatic_measure, combined_data_concordance_automatic_measure,duplicates_auto_measure,duplicates_auto_with_year_index_measure,datasets_to_always_choose], f)
        import pickle

        # combined_data_automatic, combined_data_concordance_automatic,duplicates_auto,duplicates_auto_with_year_index, datasets_to_always_choose=[],std_out_file=None
        data = pickle.load(open('intermediate_data/data_selection/automatic_method_input.pkl', 'rb'))
        combined_data_automatic = data[0]
        combined_data_concordance_automatic = data[1]
        duplicates_auto = data[2]
        duplicates_auto_with_year_index = data[3]
        datasets_to_always_choose = data[4]
        std_out_file=None
        if std_out_file is None:
                std_out_file = sys.stdout
