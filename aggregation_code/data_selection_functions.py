...
def user_input_keep_for_all_consecutive_years_that_same_datasets_are_available(selected_dataset, data_to_select_from, data_to_select_from_for_this_year, paths_dict):
    ...
    # selected_data = selected_data[selected_data.date >= data_to_select_from_for_this_year.date[0]]#fix for wanring Series.getitem treating keys as positions is deprecated. In a future version, integer keys will always be treated as labels (consistent with DataFrame behavior). To access a value by position, use ser.iloc[pos]
    
    # Filter for years after the current year
    selected_data = selected_data[selected_data.date >= data_to_select_from_for_this_year.date.iloc[0]]
    ...
    min_year = selected_data.loc[(selected_data['Year_diff'] != 1) & (selected_data['date'] != data_to_select_from_for_this_year.date.iloc[0]), 'date'].min()
